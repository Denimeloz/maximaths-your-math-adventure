...
const Dashboard = () => {
  ...
  const fetchUserData = async () => {
    ...
    // Fetch recent courses (based on user's level)
          ...
          <div className="flex items-center gap-4">
            {isAdmin && (
              <Button
                ...
                className="px-3 py-1 bg-rainbow-purple/20 text-rainbow-purple rounded-full text-sm font-body font-semibold flex items-center gap-2"
              >
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <div 
            ...
            className="card-sticker bg-gradient-to-br from-rainbow-blue/10 to-rainbow-green/10 border-rainbow-blue/30 p-6 cursor-pointer group"
          >
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-2xl bg-rainbow-blue/20 flex items-center justify-center">
                <GraduationCap className="w-8 h-8 text-rainbow-blue" />
              </div>
              <div className="flex-1">
                <h3 className="text-xl font-display text-foreground mb-1">Cours Collège</h3>
                <p className="text-muted-foreground font-body text-sm">6ème à 3ème</p>
              </div>
              <ArrowRight className="w-6 h-6 text-rainbow-blue group-hover:translate-x-1 transition-transform" />
            </div>
          </div>

          <div 
            onClick={() => navigate('/lycee')}
            className="card-sticker bg-gradient-to-br from-rainbow-purple/10 to-rainbow-pink/10 border-rainbow-purple/30 p-6 cursor-pointer group"
          >
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-2xl bg-rainbow-purple/20 flex items-center justify-center">
                <Sparkles className="w-8 h-8 text-rainbow-purple" />
              </div>
              <div className="flex-1">
                <h3 className="text-xl font-display text-foreground mb-1">Cours Lycée</h3>
                <p className="text-muted-foreground font-body text-sm">Seconde à Terminale</p>
              </div>
              <ArrowRight className="w-6 h-6 text-rainbow-purple group-hover:translate-x-1 transition-transform" />
            </div>
          </div>
        </div>
        ...
        {recentCourses.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-display text-foreground mb-4 flex items-center gap-2">
              <BookOpen className="w-6 h-6 text-rainbow-blue" />
              Cours recommandés
            </h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              {recentCourses.map((course) => (
                ...
                >
                  ...
                  <h3 className="text-lg font-display text-foreground mt-3 mb-2 group-hover:text-rainbow-blue transition-colors">
                    {course.title}
                  </h3>
                  <p className="text-sm text-muted-foreground font-body capitalize">{course.category}</p>
                  <Button variant="ghost" size="sm" className="w-full mt-4 rounded-xl">
              ...
              <div className="flex flex-wrap gap-3">
                {userBadges.map((badge) => (
                  ...
                  >
                    ...
                    <span className="font-body text-sm text-foreground">{badge.name}</span>
                  </div>
                ))}
              </div>
            ) : (
